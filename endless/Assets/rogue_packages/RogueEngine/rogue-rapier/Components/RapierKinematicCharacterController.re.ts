...
export default class RapierKinematicCharacterController extends RapierBody {
  ...
  @RE.props.num() maxSlopeClimbingDeg = 70;
  @RE.props.num() minSlopeSlidingDeg = 60;
  ...
  init() {
    ...
    this.characterController.setMaxSlopeClimbAngle(THREE.MathUtils.degToRad(this.maxSlopeClimbingDeg));
    this.characterController.setMinSlopeSlideAngle(THREE.MathUtils.degToRad(this.minSlopeSlidingDeg));
  }
  ...
  handleKinematicPositionBased() {
    ...
    if (this.isGrounded && !this.isJumping && this.playerVelocity.x !== 0 && this.playerVelocity.z !== 0) {
