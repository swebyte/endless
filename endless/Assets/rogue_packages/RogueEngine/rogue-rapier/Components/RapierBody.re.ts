...
export default class RapierBody extends RE.Component {
  ...
  onDisabled(): void {
    if (this.body) {
      this.object3d.traverse(obj => {
        const comps = RE.getObjectComponents(obj);
        comps.forEach(comp => {
          if (comp instanceof RapierCollider) {
            RogueRapier.world.removeCollider(comp.collider, true);
            comp.collider = undefined as any;
          }
        })
      });
  ...
  beforeUpdate(): void {
    ...
    this.initialized && !this.body && this.reEnable();
